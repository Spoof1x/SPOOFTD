...
function BuildingHelper:UpgradeBuilding(building)
    ...
    ParticleManager:SetParticleControlEnt(  newBuilding.particle, 1, newBuilding, PATTACH_ABSORIGIN_FOLLOW, nil, newBuilding:GetAbsOrigin(), true  )
    EmitSoundOn( "DOTA_Item.RepairKit.Target", newBuilding )

     

    newBuilding.updateHealthTimer = Timers:CreateTimer(function()
        if IsValidEntity(newBuilding) and newBuilding:IsAlive() then
              local timesUp = GameRules:GetGameTime() >= time_completed
              if not timesUp then
                if newBuilding.bUpdatingHealth then
                      if newBuilding:GetHealth() < hit_points then
                        newBuilding:SetHealth(newBuilding:GetHealth() + 1)
                      else
                        newBuilding.bUpdatingHealth = false
                     end
                end
              else
                    ParticleManager:DestroyParticle(newBuilding.particle, false)
                    newBuilding:StopSound("DOTA_Item.RepairKit.Target")
...
function BuildingHelper:StartBuilding(builder)
    ...
    local buildTime = BuildingHelper.UnitKV[unitName]["BuildTime"]
